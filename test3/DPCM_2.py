...
#2bit 量化
radio = 512 / (1 << 2)  # //量化因子  8是左向预测8bit量化，如果要进行4或2等的自行更改
for i in range(h):
    for j in range(w):
        # 左向预测8bit量化
        if j == 0:
...
plt.title('2-bit重建'), plt.axis('off')

plt.show()
# print("2-bit-psnr:", psnr(img, bgr_re))
# print("2-bit-ssim:", ssim(img, bgr_re))
psnr1 = psnr(img, bgr_re)
ssim1 = ssim(img, bgr_re)
file = open("psnr and ssim2.txt", "w")
file.write("2-bit-psnr:"+str(psnr1)+"\n")
file.write("2-bit-ssim:"+str(ssim1)+"\n")